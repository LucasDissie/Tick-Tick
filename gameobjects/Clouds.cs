...
{
    public Clouds(int levelwidth, int layer = 0, string id = "")
        : base(layer, id)
    {
        for (int i = 0; i < (levelwidth / 10 + 1); i++)
        {
            SpriteGameObject cloud = new SpriteGameObject("Backgrounds/spr_cloud_" + (GameEnvironment.Random.Next(5) + 1), 2);
            cloud.Position = new Vector2((float)GameEnvironment.Random.NextDouble() * GameEnvironment.Screen.X * levelwidth / 20  - cloud.Width / 2, 
